...
if [ "$PRISMA_PROVIDER" != "sqlite" ]; then
  ...
  run_sqlite_migrations() {
    ...
    # Community sync rebuilds a single baseline migration (*_community_init).
    # If users keep an existing SQLite volume, this migration can fail once and then block startup with P3009.
    COMMUNITY_MIGRATION=""
    COMMUNITY_INIT_COUNT=0
    OTHER_MIGRATION_COUNT=0
    for dir in /app/prisma/migrations/*; do
      [ -d "$dir" ] || continue
      name="$(basename "$dir")"
      case "$name" in
        *_community_init)
          COMMUNITY_INIT_COUNT=$((COMMUNITY_INIT_COUNT + 1))
          COMMUNITY_MIGRATION="$name"
          ;;
        *)
          OTHER_MIGRATION_COUNT=$((OTHER_MIGRATION_COUNT + 1))
          ;;
      esac
    done

    if [ "$COMMUNITY_INIT_COUNT" -eq 1 ] && [ "$OTHER_MIGRATION_COUNT" -eq 0 ] && echo "$MIGRATE_OUTPUT" | grep -Eq "P3009|already exists|table .* exists"; then
      echo "      Detected existing SQLite schema with Community baseline migration."
      echo "      Repairing migration state for: $COMMUNITY_MIGRATION"
      npx prisma migrate resolve --rolled-back "$COMMUNITY_MIGRATION" >/dev/null 2>&1 || true
      npx prisma migrate resolve --applied "$COMMUNITY_MIGRATION"
      npx prisma migrate deploy
      echo "      Migrations complete (recovered)."
      return 0
    fi
