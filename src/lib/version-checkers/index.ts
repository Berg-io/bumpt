...
import { enrichItemWithAI } from "@/lib/ai-enrichment";
...
function compareVersions(current: string | null, latest: string | null): string {
  ...
  if (
    latestParts[0] !== undefined &&
    currentParts[0] !== undefined &&
...
function deriveItemStatus(
  current: string | null,
  latest: string | null,
  cves: string[] | null | undefined
): string {
  if (cves && cves.length > 0) return "critical";
...
export async function resolveVersion(
  ...
  switch (sourceType) {
    ...
    case "endoflife":
      return checkEndOfLife(itemParams.product ?? "");
    case "npm":
...
export async function checkItemVersion(item: MonitoredItem): Promise<CheckResult> {
  if (item.sourceId) {
    ...
    const newStatus = deriveItemStatus(item.currentVersion, latestVersion, result.cves);
    ...
    getAppSetting("app_language").then((lang) =>
      enrichItemWithAI({
        itemId: item.id,
        itemName: item.name,
        itemType: item.type,
        ...
        cves: result.cves ? JSON.stringify(result.cves) : null,
        description: result.description ?? null,
        rawMetadata: result.rawMetadata ? JSON.stringify(result.rawMetadata).slice(0, 2000) : null,
        language: lang || "en",
      })
    ).catch((err) => console.error(`[AI Enrichment] ${item.name}:`, err));
