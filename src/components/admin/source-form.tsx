...
function getTestPlaceholder(type: string): string {
  const placeholders: Record<string, string> = {
    dockerhub: '{"image": "library/nginx"}',
    github: '{"repo": "grafana/grafana"}',
    appstore: '{"bundleId": "1113153706"}',
    playstore: '{"appId": "com.microsoft.teams"}',
    msstore: '{"productId": "9wzdncrfj364"}',
    chrome: '{"platform": "win64", "channel": "stable"}',
    endoflife: '{"product": "nodejs"}',
    npm: '{"package": "react"}',
    pypi: '{"package": "django"}',
    repology: '{"project": "firefox"}',
    homebrew: '{"formula": "nginx"}',
    wordpress: '{"slug": "woocommerce"}',
    winget: '{"packageId": "Google.Chrome"}',
    csv_data: '{"itemName": "Google Chrome"}',
    maven: '{"artifact": "org.apache.maven:maven-core"}',
    nuget: '{"package": "Newtonsoft.Json"}',
    gitlab: '{"project": "gitlab-org/gitlab"}',
    packagist: '{"package": "laravel/framework"}',
    crates: '{"crate": "serde"}',
    rubygems: '{"gem": "rails"}',
    goproxy: '{"module": "github.com/gin-gonic/gin"}',
    helm: '{"chart": "bitnami/nginx"}',
    snap: '{"snap": "firefox"}',
    flathub: '{"appId": "org.mozilla.firefox"}',
    terraform: '{"module": "hashicorp/consul/aws"}',
    chocolatey: '{"package": "googlechrome"}',
    pub: '{"package": "flutter"}',
    hex: '{"package": "phoenix"}',
    conda: '{"package": "conda-forge/numpy"}',
    cocoapods: '{"pod": "Alamofire"}',
    cpan: '{"distribution": "Moose"}',
    fdroid: '{"appId": "org.mozilla.fennec_fdroid"}',
    firefox_addon: '{"slug": "ublock-origin"}',
    vscode: '{"extensionId": "ms-python.python"}',
    jetbrains: '{"pluginId": "7322"}',
    openvsx: '{"extensionId": "redhat.java"}',
    aur: '{"package": "yay"}',
    ansible: '{"collection": "community.general"}',
    quay: '{"repository": "prometheus/prometheus"}',
    bitbucket: '{"repo": "atlassian/python-bitbucket"}',
...
export function SourceForm({ open, onClose, onSave, source, testOnly }: SourceFormProps) {
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      if (testParams.trim()) {
        try { params = JSON.parse(testParams); } catch { /* skip */ }
      }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="rounded-lg border p-4 space-y-3">
          ...
          {needsParams && (
            <Input
              id="testParams"
              label={t.sources.testParams}
              value={testParams}
              onChange={(e) => setTestParams(e.target.value)}
              placeholder={getTestPlaceholder(data.type)}
            />
          )}
